...
Task(subagent_type="git-history-analyzer", model="haiku", run_in_background=True,
     prompt="Find recent changes to files in [area]. Check git log for commits that might have introduced the bug...")
```
...
Task(subagent_type="executor", model="opus", run_in_background=True,
...
```
# Direct fix
Task(subagent_type="executor", model="opus",
...
1. **Reproduce** — Spawn qa-tester to create minimal reproduction
2. **Bisect** — Spawn git-history-analyzer to find the introducing commit
3. **Isolate** — Spawn architect to narrow down the root cause
4. **Fix** — Spawn executor with targeted fix
5. **Verify** — Spawn qa-tester to confirm fix and add regression test
...
|-------|-------|-------|---------|
| Diagnose | explore | haiku | Find relevant files and error patterns |
| Diagnose | architect | opus | Analyze code paths and state |
| Diagnose | git-history-analyzer | haiku | Find recent changes, bisect |
| Hypothesize | architect | opus | Synthesize and rank hypotheses |
| Fix | executor | opus | Implement targeted fix |
| Fix (speculative) | executor × 2-3 | opus | Parallel fix attempts |
| Verify | qa-tester | sonnet | Regression testing |
