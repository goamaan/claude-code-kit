/**
 * Doctor Module
 * Provides diagnostic and repair functionality for claudeops
 */
...
export function createDoctor(): Doctor {
  ...
  async function healthCheck(): Promise<'healthy' | 'degraded' | 'unhealthy'> {
    ...
    const criticalChecks = ['claudeops-dir', 'config-valid', 'active-profile'];
...
export function formatReport(report: DiagnosticReport): string {
  ...
  if (report.availableFixes.length > 0) {
    ...
    lines.push('Run "claudeops doctor --fix" to apply these fixes:');
