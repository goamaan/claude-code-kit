...
/**
 * Check if a manifest is compatible with current claude-kit version
...
export function checkCompatibility(
  ...
  if (!requires || !requires['claude-kit']) {
    // No version requirement specified, assume compatible
    return { compatible: true };
  }

  const requiredVersion = requires['claude-kit'];
  ...
  if (requiredVersion.startsWith('^')) {
    ...
    if (reqMajor !== undefined && curMajor !== undefined && curMajor < reqMajor) {
      return {
        compatible: false,
        reason: `Requires claude-kit ${requiredVersion}, but ${claudeKitVersion} is installed`,
      };
    }
    return { compatible: true };
  }
